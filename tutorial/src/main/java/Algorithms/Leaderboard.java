...
public class Leaderboard {
    ...
    static int[] climbingLeaderboard(int[] scores, int[] alice) {
        Set scoresSet = new TreeSet();
        for(int k=0;k<scores.length;k++ ){
            scoresSet.add(scores[k]);
        }
//        Set scoresSet = new HashSet<>(Arrays.asList(scores));
        List<Integer> scoreList = new ArrayList<Integer>(scoresSet);
        Collections.reverse(scoreList);

        int[] result = new int[alice.length];
        int lastPos = scoreList.size() - 1;
        ...
        for (int j = 0; j < alice.length; j++) {
            ...
            for (int i = lastPos; i >= 0; i--) {
                if (alice[j] < scoreList.get(i).intValue()) {
                    result[j] = i + 2;
