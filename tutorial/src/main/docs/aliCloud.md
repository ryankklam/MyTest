...
* 七层服务支持配置域名或者URL路径的策略, 4层服务没有
* 负载均衡的监控如果开启了黑名单或白名单，但没有添加任何ip，则（A 转发所有请求）
* SLB性能保障型，提供了可保障的性能指标，包括（A 最大连接数 B，每秒新建连接数 E 每秒查询数）
* 后端ecs之间没有数据同步功能
* SLB后端的ecs操作系统不一定需要一致
* SLB的公网或私网访问时购买时用户选择的，并不是系统自动选择
* TCP需要三次握手，而UPD属于面向非链接的协议
* 一个SLB实例最多可以绑定50个监听
* 用户在是哟负载均衡时发现http header增加了 tranfer encoding chunked的字段，但本机直接访问ecs没有这个字段是因为（A 由于7层负载基于tengine反向代理实现）
* 某用户在使用SLB，其中出现某段时间客户端访问延迟，想要快速定位异常后端服务器，可以 （C 结合阿里云日志，通过仪表巡检，分析负载均衡反应时间）
* 负载均衡的操作日志需要在 （C ActionTrail）控制台 里面查看
* 公网类型负载均衡的ip地址，（A 系统会给分一个公网服务地址）
...
* 用户进行更换系统盘后，不一定发生的是（C 原系统盘自动快照被自动删除）
...
* 快照存放在oss上，但不是用户自己的oss，是相互独立的
* 下列选项中 （A 密码）不属于实例启动模板可包含的配置信息  ， 密码是在镜像设置
* ECS不支持卸载作为数据盘使用的本地盘 （本地盘，很多操作都不行）
* 如果实例挂载了本地盘，用户可以选择的方式不包括 （D 部分重启恢复）
* ECS变更配置需要重启服务生效
* 想要更换ecs实例的公网ip，但控制台找不到更换公网ip的选项，这种问题原因可能是 （C ecs实例为vpc网络，停止实例是选择“停机不收费”）
* 磁盘序列号特性限制，只支持新创建的磁盘，已创建过的磁盘没有这个功能
* web防火墙支持阿里云或非阿里云的服务器
* 一个安全组可以包括同一个地域的的同一个专有网络VPC下的ECS实例 （安全组是地域级别的资源）
* 用户无法接入设备，也无法手动截屏，这时推荐采取的措施是 （C 通过控制台查看系统日志 D 通过控制台查看实例截图）
* windows 只有在配置较低的机器（内存少于2G）才推荐使用虚拟内存
* 本地盘与云盘的对比 （A 本地盘和云盘都支持SSD D本地盘和云盘的性能课容量大小有关）
* 阿里云强烈不建议在ecs上部署虚拟化程序，会出现不可预测的问题
* 关于云ecs说法正确的有 （A同一个ecs只能添加到一个伸缩组中  B ecs状态必须是运行中才可以添加到伸缩组  E同一伸缩组ecs必须加入负载均衡白名单）
* 应用层协议/mac地址  不属于网络五元组的一部分。
* ECS带宽临时升级支持按天升级，升级到期之后仍然按升级之前的带宽进行续费
* 每个ECS最多可以加入5个安全组，且ecs最少加入一个安全组
* 可以通过健康检查确认该实例是健康的时间窗是多久 ？ （D 7秒）
* ecs实例状态异常后，被slb实例从后段服务器中移除的时间是 （A 19秒）
* 你在创建阿里云ecs必须要选择（B 镜像）来指定新建的ecs的系统盘配置
* 因为业务需要缩短快照的制作时间，可以选择（B 本地快照） ， 快速，秒级别
* SLB的虚拟服务器组如果添加了该ecs，该ecs从经典网络迁移到vpc时，实例不会自动挂载
* 如果你想建立一个图片分享站，允许用户通过web方式进行图片上传与分享，可以使用阿里云的（B 云服务器ecs）产品配合OSS使用
* ECS部署好python环境后，通过 （B ecs监控中的进程监控） 能最简单的监控python进出的那个是否持续运行。
* 只禁止tcp 80 25 端口并不能达到禁止用户ping ecs是否在线的效果
* 若你的ECS想保持与internet的直接链接，则一定要开启公网ip或绑定eip
* 阿里云RDS不是完美兼容oracle
* 想将网站迁移到阿里云ecs，需要在阿里云接入icp备案
* （C 代码存储库）的场景不适合用共享型实例
* 阿里云支持卸载系统盘，当系统盘损坏时，可以以数据盘形式加载到其他ecs进行修复，修复完成后再作为系统盘加载到源ecs上
* 其他服务器想迁移到阿里云可以使用阿里云提供的（B SMC工具），迁移过程无需停机，不影响源业务
* 为了统一vpc下的所有ecs实例的公网出口IP，用户可以（B 为绑定原ip的ecs单独分配一块弹性网卡，并将eip绑定到弹性网卡）
...
* 用户可以通过url预热提升命中率的场景包括 （A 加速网站流量过低，导致CDN缓存节点被提前清除  B 业务压力过大 源站访问压力大 静态资源访问效率低）
* 将bucket设置成静态网页托管模式后，对静态网站域名的匿名访问 oss将返回索引页；对静态网站根域名的访问 oss将返回GetBucket结果 （选错误 B）
* OSS同城冗余能够提供机房级别的容灾能力 （B）
...
* 防盗链 通配符 使用*来代替0个或多个字符
* OSS目前不具备OCR文字识别能力
* CDN的域名配置页面，可以选择（A 源站域名 D 函数计算域名 E OSS域名）
* OSS的bucket版本控制处于开启，就不能关闭。开启后每一个版本的object都会保存，占用空间
* CDN的组建包括 （B LVS C tengine D swift），lvs4层负载均衡，tengine7层负载均衡，swift做http缓存
* CDN同名更新，用户不主动提交请求的话，CDN不会自动去刷新请求
* CDN与ECS访问，不管是否同一地域都会产生流量费
* CDN实际产生的流量比应用层统计到的要高。是两个原因 1： TCP/IP的包头的消耗 2：TCP重传
* 公共云计算中使用的存储并不是共享存储，而是分布式私有存储
* 发现有黑客恶意下载公司的正版视频，在国外社区里分发。从技术上可以(B 启用CDN的URL鉴权功能)
* ECS与OSS走内网流量不收费，但每次的请求次数还是计费的
* 当OSS的bucket是私有时，OSS分享链接采取的安全机制是 （C 在管理控制台获取文件访问url时设置链接分享有效时间，超过规定时间则无法下载）
* CDN网站最好做动静分离
* CDN的不支持对IP的直接加速，需要绑定域名
* OSS可以满足严格的电子记录保留要求，用户可以设置（B 基于时间）的保留策略
...
* 同一专有网络不同交换机下的子网内网互通
* 不同专有网络之间完全隔离，可以通过对外映射的ip（弹性公网ip或NATip）来互联
* EIP在ecs操作系统控制台不可见，需要通过api才可以查询到
* VPC的高级功能包括 （A 网络ACL B流日志 D自定义路由表）
* 修改私网ip需要保证ecs实例处于已停止状态
* 默认安全组，默认不打开（D http 80端口）

##ESS弹性伸缩
* 弹性自愈 - 就是（B 如果伸缩组某台ecs不是运行中，则该ecs会被移出伸缩组，同时自动创建一台新的ecs添加进伸缩组）
* 伸缩组包含 （A 伸缩配置 B 伸缩规则 C 伸缩活动）
* 在启用弹性伸缩之前，需要设置（A 伸缩配置）条件，可实现在业务增长时自动添加ecs实例，在需求下降时自动减少ecs实例以节约成本
* 伸缩规则定义了具体的扩展或收缩工作 如 （A 加入或移除3个云服务的ECS实例）
* 使用伸缩组内的实例配置信息自动创建ECS，实例配置信息支持实例的规格，用户自定义数据及 （A 操作系统）
* 伸缩活动不可中断
* 在冷却时间内，伸缩组会拒绝由云监控发起的伸缩活动请求，对其他的类型的任务则会立即执行，绕过冷却时间
* 弹性伸缩生命周期挂钩支持的配置方式不包括（D. OSS主板） 
...
* 弹性伸缩支持多种配置 如（A标签 B SSH密钥对 D实例RAM角色 F实例自定义数据）帮你高效 灵活地自定义ecs的实例配置
* 在非强制删除条件下，要满足以下两个条件才可以删除伸缩组 （A 没有任何伸缩活动正在执行  B 伸缩组内ecs数量为0）
* 伸缩组扩展必要的是 （B ECS的实例分配IP C 启动ecs实例）
* 如何实现实例的自动加入和移除redis白名单 （A 弹性伸缩生命周期挂钩  B OOS模版）
* 将公网ip转换成弹性公网ip 操作成功后 （A 公网带宽值与源ecs保持一致  C EIP采用流量计费的方式）
* 伸缩组有3个状态 （A Active C deleting E Inactive）
* 手工添加的ecs实例，不受伸缩配置中定义的规格限制
* 如果弹性伸缩的ecs无法加入到RDS的访问白名单，则自动创建的ecs会被回滚释放，手动创建的则不会回收释放
* 弹性伸缩可以把多种模式混合使用，比如同一个伸缩组可以同时设置固定数量模式，定时模式以及动态模式
* 弹性伸缩支持数据盘的弹性伸缩
* 弹性伸缩要创建10台ecs，9台成功一台失败，则系统不会再继续创建剩余的ecs，而是显示部分成功
...
* 防暴力密码破解是云骑士的功能，不是web防火墙功能
* 防web应用系统密码破解是属于web防火墙的功能，是需要收费的
...
* 云监控 支持4种报警方式  电话 短讯 邮件 钉钉机器人），记住微信 qq一定不可以。。。
...
* DDoS的攻击类型包括 (A：传输层 B：web应用层 C：畸形报文)
* 调整DDos防护阙值并不会导致服务中断
* DDoS的默认防护是5G，意思是小于5G，不是次次都是5G
...
* 数据风控是利用大数据的处理能力，通过风险决策引擎，解决企业账户、交易等关键环节存在的欺诈行为
* 阿里云的云盾先知计划，为企业提供的是 （A 收集情报漏洞）
* 阿里云的云盾先知计划目的是帮助企业建立私有的 （B 应急响应中心）
* 防止本地ip频繁访问阿里云而被屏蔽的情况，可以通过云盾设置（A ip白名单）
* 云盾加密服务的密钥是由（B 客户） 负责保管的
* 云盾的风险识别api会返回识别结果，但是对风险不会自动处理
* 在ecs上安装了云服务器的云安全中心客户端，通过 （D 安装验证key与阿里云账户互联）
* 云安全中心，错误的是（C 卸载了agent插件的ecs不受云安全中心的保护，但控制台仍然会显示资产的漏洞等信息）

* 希望将用户的访问重定向到https，最简单的实现方式是 （B 配置一个新的监听，并开启监听转发功能）
* 某大型互联网用户，网上资产比较多，但是不清楚自己的资产那些联网了，存在较大安全隐患，阿里云的 （B 安全管家）最适合解决这个问题
* 超大文本文件的结构化分析，实现分钟级处理，推荐的方案是（A 把数据上载到OSS上 B 通过maxcompute的oss文件加载功能，进行结构化分析），不要选data analyst
* CAP理论 即分布式系统不可能同时满足 （A 一致性 B 可用性 E 分区容错性）
* VBR说法正确的是 （A 支持BGP协议 D 决定物理专线端口模式 三层路由接口或基于VLAN的三层子接口）
