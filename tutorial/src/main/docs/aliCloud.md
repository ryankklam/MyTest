...
* 为了统一vpc下的所有ecs实例的公网出口IP，用户可以（B 为绑定原ip的ecsd单独分配一块弹性网卡，并将eip绑定到弹性网卡）
...
* 用户可以通过url预热提升命中率的场景包括 （A 加速网站流量过低，导致CDN缓存节点被提前清楚  B 业务压力过大 源站访问压力大 静态资源访问效率低）
...
* 云安全中心，卸载了agent插件的ecs不受云安全中心的保护，但控制台仍然会显示资产的漏洞等信息
